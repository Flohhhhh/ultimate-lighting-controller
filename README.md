...
1. All changes that affect the `ulc.lua` config in any way must be 100% backwards compatible. That is, a `ulc.lua` file that was created on the first day ULC was released must still work today.
   ...
   3. For this reason, it's imperative that new features be well thought-out in terms of the format of their configuration in the `ulc.lua` files.
